...
此工程为 [唯杰](https://vjmap.com)  [WebCAD](https://vjmap.com/app/webcad/) 示例的所有源代码，每个示例都是独立的 HTML+JS 文件，可单独运行。

## 目录结构

```
playground/
├── index.html          # 入口页（中文/English 切换）
├── README.md
├── js/
│   └── vjcad-lib.umd.js   # WebCAD 核心库
├── zh/                 # 中文示例（文件夹和文件名含中文）
│   ├── index.html
│   ├── 01basic_基础示例/
│   │   ├── 01init.js
...
由于 `file://` 协议存在跨域限制，如果会导致资源无法加载，需要通过 HTTP 服务器访问。

### 方法一：live-server（需要 Node.js）
...
2. 用 VS Code 打开本目录
3. 右键任意 `.html` 文件，选择 **Open with Live Server**
...
- **HTML 文件** — 页面骨架，加载 `vjcad-lib.umd.js` 库和对应的 JS 文件
- **JS 文件** — 示例逻辑代码，包含环境配置（serviceUrl / accessToken）和完整的运行代码

`zh/` 和 `en/` 目录下的示例内容完全相同，区别仅在于文件夹和文件命名是否包含中文。

## 重新生成

示例源码位于 `playground` 工程中，如需重新生成：
